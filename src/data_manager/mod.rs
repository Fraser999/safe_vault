...
impl<'a> DataManager<'a> {
    pub fn new<'b>(routing: &'b mut Routing) -> DataManager<'b> {
        DataManager {
            routing: routing,
    ...
    pub fn handle_get(&mut self, request: &RequestMessage) {
        ...
        for fetch_name in fetching_list.iter() {
            ...
            for pmid in self.database.get_pmid_nodes(fetch_name) {
                ...
                self.routing.send_get_request(src, dst, content);
    ...
    pub fn handle_put(&mut self, data: &ImmutableData) {
        ...
        for pmid in dest_pmids {
            ...
            self.routing.send_put_request(src, dst, content);
        }
    }
    ...
    pub fn handle_churn(&mut self, close_group: Vec<XorName>, churn_node: &XorName) {
        ...
        if let Ok(serialised_stats) = serialise(&[data_manager_stats.clone()]) {
            self.routing.send_refresh_request(STATS_TAG, Authority::NaeManager(churn_node.clone()),
