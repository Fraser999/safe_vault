...
pub struct PmidManager<'a> {
    routing: &'a mut Routing,
    database: database::Database,
}

impl<'a> PmidManager<'a> {
    pub fn new<'b>(routing: &'b mut Routing) -> PmidManager<'b> {
        PmidManager { routing: routing, database: database::Database::new() }
    }

    pub fn handle_put(&mut self, data: &ImmutableData, pmid_node_name: XorName) {
        ...
        let _ = self.routing.send_put_request(src, dst, content);
    }
