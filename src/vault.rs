...
pub struct Vault<'a> {
    ...
    data_manager: DataManager<'a>,
    maid_manager: MaidManager<'a>,
    pmid_manager: PmidManager<'a>,
    pmid_node: PmidNode<'a>,
    sd_manager: StructuredDataManager<'a>,
...
impl<'a> Vault<'a> {
    pub fn run() {
        ...
        // TODO - Keep retrying to construct new Routing until returns Ok() rather than using unwrap?
        let mut routing = unwrap_result!(Routing::new(routing_sender));
        Vault::new(&mut routing, routing_receiver, None, None).do_run();
    }

    fn new<'b:'a>(routing: &'b mut Routing,
                  routing_receiver: mpsc::Receiver<Event>,
                  app_event_sender: Option<mpsc::Sender<Event>>,
                  should_stop: Option<Arc<AtomicBool>>)
                  -> Vault<'a> {
        ::sodiumoxide::init();
        Vault {
            routing: routing,
            data_manager: DataManager::new(routing),
            maid_manager: MaidManager::new(routing),
            pmid_manager: PmidManager::new(routing),
            pmid_node: PmidNode::new(routing),
            sd_manager: StructuredDataManager::new(routing),
            churn_timestamp: SteadyTime::now(),
            receiver: routing_receiver,
        ...
        match (&request.src, &request.dst, &request.content) {
            ...
            (&Authority::Client{ .. },
             ...
             &RequestContent::Get(DataRequest::ImmutableData(_, _))) => {
                self.data_manager.handle_get(&request)
            }
            (&Authority::Client{ .. },
             &Authority::NaeManager(_),
             &RequestContent::Get(DataRequest::StructuredData(_, _))) => {
                self.sd_manager.handle_get(&request)
            }
            (&Authority::NaeManager(_),
             &Authority::ManagedNode(_),
             &RequestContent::Get(DataRequest::ImmutableData(_, _))) => {
                self.pmid_node.handle_get(&request)
            }
            // ================== Put ==================
            (&Authority::Client{ .. },
            ...
            (&Authority::Client{ .. },
             ...
             &RequestContent::Put(Data::StructuredData(_)))=> {
                self.maid_manager.handle_put(&request)
            }
            (&Authority::ClientManager(_),
             &Authority::NaeManager(_),
            ...
            (&Authority::NaeManager(_),
             ...
             &RequestContent::Put(Data::ImmutableData(ref data))) => {
                self.pmid_manager.handle_put(data, pmid_node_name)
            }
