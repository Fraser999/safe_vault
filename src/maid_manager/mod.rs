...
pub struct MaidManager<'a> {
    routing: &'a mut Routing,
    database: database::Database,
}

impl<'a> MaidManager<'a> {
    pub fn new<'b>(routing: &'b mut Routing) -> MaidManager<'b> {
        MaidManager { routing: routing, database: database::Database::new() }
    }

    pub fn handle_put(&mut self, request: &RequestMessage) {
        ...
        if !self.database.put_data(request.src.get_name(), data.payload_size() as u64) {
            ...
            let _ = self.routing.send_put_response(src, dst, content);
            return
        }

        let dst = Authority::NaeManager(data.name());
        let content = RequestContent::Put(data);
        self.routing.send_put_request(request.dst.clone(), dst, content);
    }
