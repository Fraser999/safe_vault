...
pub struct StructuredDataManager<'a> {
    routing: &'a mut Routing,
    chunk_store: ChunkStore,
}

impl<'a> StructuredDataManager<'a> {
    pub fn new<'b>(routing: &'b mut Routing) -> StructuredDataManager<'b> {
    ...
    pub fn handle_get(&mut self, request: &RequestMessage) {
        ...
        let _ = self.routing.send_get_response(request.dst.clone(), request.src.clone(), content);
    }
    ...
    pub fn handle_churn(&mut self, churn_node: &XorName) {
        ...
        for name in names {
            ...
            self.routing.send_refresh_request(ACCOUNT_TAG, src, data, churn_node.clone());
        ...
        if type_tag == &ACCOUNT_TAG {
            ...
            for name in names {
                if *our_authority.get_name() == name {
                    ...
                    self.routing.send_refresh_request(ACCOUNT_TAG, our_authority.clone(), data,
                                                      churn_node.clone());
                }
            }
